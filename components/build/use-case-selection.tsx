...
export function UseCaseSelection({ onNext }: UseCaseSelectionProps) {
    ...
    const handleContinue = () => {
        ...
        if (selectedCategory && selectedVariant) {
            ...
            const variant = category.variants[selectedVariant as keyof typeof category.variants] as any
            if (variant) {
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                <Card
                    className={`group cursor-pointer border-2 p-6 transition-all hover:border-accent ${selectedCategory === "custom" ? "border-accent bg-accent/5" : "border-border"
                        }`}
                    onClick={() => {
                ...
                {Object.entries(usecasePresets).map(
                    ([key, category]) =>
                        key !== "custom" && (
                            <Card
                                key={key}
                                className={`group cursor-pointer border-2 p-6 transition-all hover:border-accent ${selectedCategory === key ? "border-accent bg-accent/5" : "border-border"
                                    }`}
                                onClick={() => {
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        {Object.entries(usecasePresets[selectedCategory as keyof typeof usecasePresets].variants).map(
                            ([key, variant]) => (
                                <Card
                                    key={key}
                                    className={`cursor-pointer border-2 p-4 transition-all hover:border-accent ${selectedVariant === key ? "border-accent bg-accent/5" : "border-border"
                                        }`}
